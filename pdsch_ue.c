...
int srslte_ue_dl_ul_find_and_decode(srslte_ue_dl_t*     q,
				 ...
				 bool                acks[SRSLTE_MAX_CODEWORDS])
{
  ...
  for (uint32_t i = 0; i < mi_set_len && !ret; i++) {
    ...
    ul_ret =  srslte_ue_dl_find_ul_dci(q,sf,cfg,pdsch_cfg->rnti,&dci_ul);
  ...
  if (ret == 1||ul_ret) {
    ...
    srslte_dci_ul_info(&dci_ul, str, 512);
    INFO("PDCCH_ul: %s, snr=%.1f dB\n", str, q->chest_res.snr_db);
...
int main(int argc, char **argv) {
  ...
  for(int sf_idx=0;sf_idx<10;sf_idx++){
    for(int i =0; i<16;i++){
      printf("sf_idx: %d, ncce: %d, L: %d\n",sf_idx, ue_dl.current_ss_ue[0][1][sf_idx].loc[i].ncce, ue_dl.current_ss_ue[0][1][sf_idx].loc[i].L);
    }
  }
  ...
  while (!go_exit && (sf_cnt < prog_args.nof_subframes || prog_args.nof_subframes == -1)) {
    ...
    if (ret == 1) {
      ...
      switch (state) {
		...
		//printf("now decode!!\n");
		n = srslte_ue_dl_ul_find_and_decode(&ue_dl, &dl_sf, &ue_dl_cfg, &pdsch_cfg, data, acks);
                if (n > 0) {
		  // printf("detected\n");
                  nof_detected++;
                  last_decoded_tm = tm;
