...
int update_raul() {
  uint32_t     L_rb          = 2;
  ZERO_OBJECT(dci_ul);
  dci_ul.format = SRSLTE_DCI_FORMAT0;
  dci_ul.freq_hop_fl = -1;//disabled
  dci_ul.type2_alloc.riv = srslte_ra_type2_to_riv(L_rb, 0, cell.nof_prb);
  dci_ul.tb.mcs_idx = mcs_idx;
  dci_ul.rnti = UE_CRNTI;
  return 0;
}
...
int main(int argc, char **argv) {
  ...
  ZERO_OBJECT(ul_sf);
  bzero(&pusch_cfg, sizeof(srslte_pusch_cfg_t));
  ...
  if (srslte_ra_ul_dci_to_grant(&cell, &ul_sf, &ul_hopping, &dci_ul, &pusch_cfg.grant)) {
  ...
  while ((nf < nof_frames || nof_frames == -1) && !go_exit) {
	...
	if (mch_table[sf_idx] == 0 || mbsfn_area_id < 0) { // PDCCH + PDSCH
	  ...
	  printf("Putting DCI to location: n=%d, L=%d\n", locations[sf_idx][0].ncce, locations[sf_idx][0].L);
          dci_dl.location = locations[sf_idx][0];
	  ...
	  /******************************************************/
	  for(i=0;i<16;i++){
	    printf("sf_idx:%d, DCI %d location: n=%d, L=%d\n",sf_idx,i,locations[sf_idx][i].ncce, locations[sf_idx][i].L);
	  }
	  
       	  dci_ul.location = locations[sf_idx][3];
	  printf("Putting UL DCI to location: n=%d, L=%d\n", locations[sf_idx][3].ncce, locations[sf_idx][3].L);
	  /* Encode PDCCH for UL Grant(dci_ul to be set)*/
      	  if (srslte_dci_msg_pack_pusch(&cell, &dl_sf, NULL, &dci_ul, &dci_msg)) {
      ...
      srslte_vec_save_file("srslte_rf_txrx_tone", output_buffer, sizeof(cf_t) * sf_n_samples);
